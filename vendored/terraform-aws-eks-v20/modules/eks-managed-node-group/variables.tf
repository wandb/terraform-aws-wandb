...
variable "name_prefix_separator" {
  description = "String inserted between the launch template / node group name and the random suffix when use_name_prefix is true. Default \"-\" matches stock v20 behavior; set to \"\" to preserve a v17-era name pattern that did not include a separator. Custom patch applied for v17->v20 in-place upgrades; not present in upstream terraform-aws-modules/eks/aws."
  type        = string
  default     = "-"
}
